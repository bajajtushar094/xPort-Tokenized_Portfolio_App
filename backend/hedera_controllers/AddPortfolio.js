...
async function submitMessage(topicId) {
    ...
    console.log("Private Key from global variable: ", myPrivateKey);
    // letvalue = { name: "Logan", age: 21, location: "London" };
    letresult = JSON.stringify(myPortfolio);
...
//pass arguments
async function submitPrivateMessage(accountId, privateKey, portfolio) {

    myAccountId = AccountId.fromString(accountId);
    myPrivateKey = PrivateKey.fromString(privateKey);
    myPortfolio = portfolio.toObject();
...
module.exports = {submitPrivateMessage};